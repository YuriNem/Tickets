...
export default class App extends React.Component {
  componentDidMount() {
    ...
    window.addEventListener("resize", event =>
      asyncOffMobileSidebar({ width: event.target.screen.width })
    );
    asyncGetTickets();
  }
              ...
              ? tickets
                  ...
                  .filter(ticket => {
                    return [allStops, noStops, oneStop, twoStops, threeStops]
                      ...
                      .map(
                        filter =>
                          filter.index === -1 || filter.index === ticket.stops
                      )
                      .filter(state => state).length;
                  })
                  .map(ticket => (
                    <TicketContainer {...ticket} key={uniqueId()} />
                  ))
              : null}
          </div>
        </div>
        {!mobileSidebar ? (
          <button className="app__button" onClick={asyncChangeSidebarView}>
            Фильтровать
          </button>
        ) : null}
      </div>
